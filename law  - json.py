...
input_df = pd.read_csv('D:\\Temp\\2021\\processed\\2011stuff.csv',nrows=20)
...
role_ass = [ {"role": "system", "content":
			"你是一名非常专业的中国律师，你现在的工作是从法律文书中分类出一些信息and format them for JSON output,这是一份汇报你无需使用礼貌用语，你必须做到能把所提供的信息以段落的形式陈述出来,你的回复需要尽可能的长，你的回复长度需要尽量达到你所被规定的最大token数量，你的工作是陈述而不是总结所以请不忽略任何细节，你的回复必须足够的长，你的回复必须遵守以下格式：/n “公诉机关”, “被告人” ，“原告”，“案件起因经过”，“被告辩护”，“法院裁决结果以及理由”， “引用的法规”， 以下是全部的法律文书，你的信息提取必须做到非常详细，保证能概括到所有提供的信息，如果有信息未被覆盖，请写“无” "} ]
def extraction(case_title, case_type, instrument,case_process):
	message = "以下是文书主体\n标题："+case_title+"\n"+"案件类型："+case_type+"\n文书内容："+instrument+"\n法律依据："+case_process
...
for index, row in input_df.iterrows():
    ...
    case_title = row['标题']
    case_type = row['案件类型']
    case_process = row['审理程序']
    cited_law = row['法律依据']
    case_year = row['裁判年份']
    
    #print(case_information)
    
    extraction(case_title,case_type,case_information,case_process)
